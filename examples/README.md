...
- `minimal/` — a complete valid pass through every layer: one source per source type (document, publication, data), one distillate each, one claim grounded across all three, one deliverable paragraph with a grounded footnote and a posit. Its `knowledge/state.md` is the inventory register that records the representations and distillates. `python tools/validate.py examples/minimal --run-computations` passes clean and without a warning.
