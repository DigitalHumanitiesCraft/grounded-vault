...
- Chapter scope: `python tools/validate.py . --chapter 40_output/<slug>` judges one chapter and, transitively, the assertions, distillates and Markdown representations it grounds in, so that a chapter can be reported ready while other parts of the vault are still in progress. The scope walk follows only anchors pointing one layer down, so a neighbouring branch of the vault stays out of the verdict. Checks that are decidable only over the whole vault stay out too, the inventory obligation and the vault-level warnings, and the run names them in its closing lines. In this mode an undeclared warning fails the run alongside an error, because the question the run answers is whether this chapter is ready for acceptance.
